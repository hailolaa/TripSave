...
export class WalmartScraperService extends OxylabsBaseService {
  ...
  async search(query: string, zip?: string): Promise<ScrapedProduct[]> {
    ...
    try {
      ...
      this.logger.error(`Walmart scrape failed: ${err.message}`);
      return [];
    }
  }
  ...
  private parse(html: string): ScrapedProduct[] {
    ...
    try {
      ...
      if (nextDataMatch) {
        ...
        for (const stack of itemStacks) {
          ...
          for (const item of items) {
            const price = item.priceInfo?.currentPrice?.price || item.price || item.itemPrice || 0;
      ...
      this.logger.debug(`Walmart JSON parse failed, falling back to regex: ${e.message}`);
    }
    ...
    if (products.length === 0) {
      ...
      let pMatch, tMatch;
      while ((pMatch = priceRegex.exec(html)) && (tMatch = titleRegex.exec(html))) {
        products.push({
          store: 'Walmart',
