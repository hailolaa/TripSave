...
export class ComparisonService {
  ...
  private async formatScrapedForUI(
    ...
    const isGasSearch = ['gas', 'fuel'].includes(normalizedQuery);
  ...
  async compareGasStations(
    ...
    const comparisons = await Promise.all(
      nearbyStores.map(async (ns) => {
        ...
        const pricePerGallon = Number(gp.regular_price);
  ...
  private resolveFallbackProductImage(category?: string, productName?: string): string {
    ...
    const fallbackImages: Record<string, string> = {
      ...
      eggs:
        'https://images.pexels.com/photos/162712/pexels-photo-162712.jpeg?auto=compress&cs=tinysrgb&w=400',
      rice:
